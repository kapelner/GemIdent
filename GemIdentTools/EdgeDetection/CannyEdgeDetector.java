package GemIdentTools.EdgeDetection;
...
public class CannyEdgeDetector {
	...
	public IntMatrix getEdgesScores() {
		return edgesImage;
	}
...
//			data[i] = data[i] > 0 ? BoolMatrix.WhiteRGB : BoolMatrix.BlackRGB;
//		}
//	}
	...
	private void writeEdges(int pixels[]) {
		...
		if (edgesImage == null) {
			edgesImage = new IntMatrix(width, height);
		}

		for (int i = 0; i < width; i++){
			for (int j = 0; j < height; j++){
//				try {
					edgesImage.set(i, j, pixels[j * width + i]);
//				}
